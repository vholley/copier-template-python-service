...
2. Draft `intent.md` from `templates/intent.md`, or, when the item's workflow is `project` (`make status` names it), from `templates/intent-project.md` for a new project and `templates/intent-process.md` for a change to the process itself. Fill What and Why from the engineer's words. Non-goals is mandatory: list at least one thing this item will not do. Done means: observable outcomes the requester would accept.
