...
IMPLEMENT_WORDS = re.compile(r"\b(implement|build|write the code|code it|add the (function|endpoint|feature)|make it work)\b", re.I)
BEHAVIOR_WORDS = re.compile(r"\b(add|change|implement|remove|support|refactor|migrate|new)\b.*\b(flag|option|endpoint|feature|behavior|command|field|api|model)\b", re.I)
TYPO_WORDS = re.compile(r"\b(typo|docstring|comment|rename|formatting|readme|wording)\b", re.I)
ACCEPT_RE = re.compile(r"Accept(-Decision)?:|accept\(work-|accept\([^)]*\): opt-out|\bmake accept\b(?!-prepare)")
...
def _protected(repo: Path, rel: str) -> bool:
    ...
    patterns = ["working/architecture/**", "working/standards/**", ".claude/**", "scripts/**", ".github/**"]
    text = (repo / paths.CONSTRAINTS_FILE).read_text() if (repo / paths.CONSTRAINTS_FILE).exists() else ""
...
def _cause_confirmed(d: Path) -> bool:
    diag = d / "diagnosis.md"
    return diag.exists() and re.search(r"^\s*status:\s*confirmed\s*$", diag.read_text(), re.M) is not None


def _exit(rule: str, what: str, why: str, next_: list[str], anchor: str) -> tuple[int, str, str]:
    return 2, "", block.render(rule, what, why, [*next_, "make opt-out  (leave the process for this branch)"], f"working/README.md#{anchor}") + "\n"
...
def prompt(payload: dict[str, object], repo: Path) -> tuple[int, str, str]:
    ...
    if st is None:
        if BEHAVIOR_WORDS.search(text) and not TYPO_WORDS.search(text):
            return 0, "This looks like a behavior change. Run /start before editing source so the work item exists.\n", ""
        return 0, "", ""
    ...
    if IMPLEMENT_WORDS.search(text):
        ...
        return _exit(
            "stage", f"implementation asked for while {item} is at stage {st.stage}",
            "code is written only after the spec (or diagnosis) is accepted",
            [cmd, "make status"], "stages",
        )
    return 0, "", ""
...
def edit(payload: dict[str, object], repo: Path) -> tuple[int, str, str]:
    ...
    if rel.endswith("state.json") and rel.startswith(".work/"):
        return _exit("state", f"{rel} is written only by stage_state", "state.json is a derived cache with a checksum",
                     ["make status", "make rebuild ID=<id>"], "state")
    if _protected(repo, rel):
        return _exit("protected", f"{rel} is a protected path", "process-defining files change only through a reviewed pull request",
                     ["describe the change to the engineer; they edit it in their own session"], "protected")
    item, st = _bound(repo)
    if st is None or st.workflow == "spike" or not SRC_RE.match(rel):
        return 0, "", ""
    if st.workflow == "change-defect" and st.stage == "diagnose" and not _cause_confirmed(state.item_dir(repo, str(item))):
        return _exit("diagnose", f"source edit while the cause of {item} is not confirmed",
                     "a fix is written only after a discriminating test confirms the cause",
                     ["/diagnose  (record the evidence; mark the cause confirmed)", "make status"], "diagnosis")
    if st.stage in STAGES_BEFORE_CODE:
        cmd = status.STAGE_COMMANDS.get(st.stage, ["make status"])[0]
        return _exit("stage", f"source edit while {item} is at stage {st.stage}",
                     "code is written only after the spec is accepted", [cmd, "make status"], "stages")
    return 0, "", ""
...
def bash(payload: dict[str, object], repo: Path) -> tuple[int, str, str]:
    ...
    if (is_commit and ACCEPT_RE.search(cmd)) or re.search(r"\bmake\s+accept\b(?!-prepare)", cmd):
        return _exit("accept", "an acceptance commit from the agent",
                     "acceptances are signed by the engineer in their own shell; the agent only prepares them",
                     ["make accept-prepare STAGE=<stage>  (then tell the engineer: make accept STAGE=<stage>)"], "signing")
    if is_commit and re.search(r"-F\s+\S*accept-[\w-]+\.msg", cmd):
        return _exit("accept", "committing a prepared acceptance message from the agent",
                     "the engineer signs it", ["tell the engineer: make accept STAGE=<stage>"], "signing")
    _ = repo
    return 0, "", ""
...
def ask(payload: dict[str, object], repo: Path) -> tuple[int, str, str]:
    ...
    substantial = sum(1 for q in questions if isinstance(q, dict) and len(str(q.get("question", "")).splitlines()) > limit) if isinstance(questions, list) else 0
    if n > 3:
        return _exit("ask", f"{n} questions in one turn", "at most three small questions per turn; the engineer's attention is budgeted",
                     ["split the ask across turns"], "review")
    if substantial > 1:
        return _exit("ask", f"{substantial} substantial questions in one turn", "at most one substantial question per turn",
                     ["ask the first now, the next after the answer"], "review")
    return 0, "", ""
...
def _red_stage_problems(repo: Path, d: Path) -> list[str]:
    ...
    env = dict(os.environ)
    env["PYTHONPATH"] = os.pathsep.join(str(p) for p in [*repo.glob("libs/*/src"), *repo.glob("apps/*/src")])
    for entries in tmap.values():
        for e in entries:
            t = str(e.get("test"))
            r = subprocess.run([sys.executable, "-m", "pytest", "-q", "-p", "no:cacheprovider", "-o", "addopts=", t],  # noqa: S603
...
def stop(_payload: dict[str, object], repo: Path) -> tuple[int, str, str]:
    ...
    problems: list[str] = []
    dirty = [ln[3:] for ln in gitx.run(repo, "status", "--porcelain", "--", "libs", "apps", check=False).splitlines()]
    if dirty:
    ...
    if retries >= cap and "escalated" in state.exits(st.stage):
        ...
        progress.write_text(existing + f"\n## escalated after {retries} blocks\n" + "".join(f"- {p}\n" for p in problems))
        return 2, "", block.render("stop", f"{item} escalated after {retries} blocks: " + "; ".join(problems),
                                   "a human decides how to continue", ["make status  (lists: implement, spec, abandon)"],
                                   "working/README.md#escalated") + "\n"
    return _exit("stop", "; ".join(problems), "a turn ends only with committed, green, evidenced work",
                 ["fix the items above, then: make green", f"(block {retries} of {cap}; the item escalates at {cap})"], "green")
...
def subagent_stop(payload: dict[str, object], repo: Path) -> tuple[int, str, str]:
    ...
    if item and not (state.item_dir(repo, item) / "evaluator-report.md").exists():
        return _exit("evaluator", "the evaluator ended without writing evaluator-report.md",
                     "the evaluator's only output is its report", ["write .work/<id>/evaluator-report.md, then stop"], "evaluator")
    return 0, "", ""
...
def post_edit(payload: dict[str, object], repo: Path) -> tuple[int, str, str]:
    ...
    r = subprocess.run(["uv", "run", "ruff", "check", "--output-format", "concise", path], cwd=repo, capture_output=True, text=True, check=False)  # noqa: S603, S607
    if r.returncode != 0:
...
def main(argv: list[str]) -> int:
    ...
    if not argv:
        print(block.render("usage", "an event name is required", "see docstring", ["python -m delivery.hooks stop"], "working/README.md#hooks"), file=sys.stderr)
        return 2
